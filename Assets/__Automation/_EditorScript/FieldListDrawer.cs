...
public class FieldListDrawer : PropertyDrawer {
	...
	public override void OnGUI( Rect position, SerializedProperty property, GUIContent label )
    {
		...
		// GameObject gm = (GameObject) GetTargetObjectOfProperty( property , gmObjectString ) ; 
    ...
    public void GetAllFunction(Component comp,ref List<string> fieldList,BindingFlags flags, bool addVoidReturnType = false, bool addValueReturnType = false)
    {
        ...
        if(comp.GetType() == typeof(Transform) || comp.GetType() == typeof(RectTransform))
        {
            ...
            // gmMethods = GetExtensionMethods( comp.gameObject.GetType().Assembly, comp.gameObject.GetType()).ToArray();
            if(gmMethods == null ) 
        ...
        // compMethods = GetExtensionMethods( comp.GetType().Assembly, comp.GetType()).ToArray();
        if(compMethods == null) 
	...
	public void GetAllFields(Component comp,ref List<string> fieldList, BindingFlags flags, bool onlyNumeric)
    {
        ...
        for (int i = 0; i < compFields.Length; i++)
        {
            ...
            fieldList.Add(lastString + "/" + compFields[i].Name);
            // propertyInfoList.Add(new PropertyDescription(fieldInfo.Name, comp));
    ...
    public void GetAllProperty(Component comp, ref List<string> fieldList, BindingFlags flags, bool onlyCanWrite = false, bool ignoreGameObject = false, bool onlyNumeric = false)
    {
        ...
        if(!ignoreGameObject && ( comp.GetType() == typeof(Transform) || comp.GetType() == typeof(RectTransform) ) )
        {
            ...
            for (int i = 0; i < gmProperty.Length; i++)
            {
                ...
                // propertyInfoList.Add(new PropertyDescription(gmProperty[i].Name, null, true));
            }
        }
        string[] splitString = comp.GetType().ToString().Split('.');
        string lastString = splitString[splitString.Length - 1]; 
        for (int i = 0; i < compProperty.Length; i++)
        {
            ...
            fieldList.Add(lastString + "/" + compProperty[i].Name);
            // propertyInfoList.Add(new PropertyDescription(compProperty[i].Name, comp, true));
        }
    }
        ...
        Type extendedType)
    {
        ...
        // Debug.Log(extendedType.ToString());

        // Type[] assemblyType = assembly.GetTypes();
        // foreach (var item in assemblyType)
        // {
        //     if(item.IsSealed && !item.IsGenericType && !item.IsNested)
        //     {
        //         MethodInfo[] allMethods = item.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
        //         foreach (var item2 in allMethods)
        //         {
        //             if( item2.IsDefined(typeof(ExtensionAttribute), false) ){
        //                 Debug.Log(item2.GetParameters()[0].ParameterType.ToString());
        //             }
        //         }
        //     }
        // }
