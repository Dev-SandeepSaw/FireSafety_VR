...
public static class ReflectionExtension{
    ...
    // public static void GetComponent(ref GameObject gm, ref Component comp, string compName)
    // {
    //     if(Type.GetType(compName) != null)
    //         comp = gm.GetComponent(Type.GetType(compName));
    //     else
    //         comp = gm.GetComponent(compName);
        
    //     if(comp == null)
    //     {
    //         Component[] allComp = gm.GetComponents<Component>();
    //         for (int i = 0; i < allComp.Length; i++)
    //         {
    //             if(allComp[i].ToString().Contains(compName))
    //                 comp = allComp[i];
    //         }
    //     }
    // }
    // public static Type GetComponentType(GameObject gm, string compName)
    // {
    //     Component comp;
    //     if(Type.GetType(compName) != null)
    //         comp = gm.GetComponent(Type.GetType(compName));
    ...
    public static void putSelectedValue(ref MethodInfo methodInfo, UnityEngine.Object unityObj, string selectedField)
    {
        ...
        // Debug.Log(unityObj.GetType().GetMethod(functionName));

        MethodInfo[] compMethods = unityObj.GetType().GetMethods(flagsForFunc);
        for (int i = 0; i < compMethods.Length; i++)
        {
            string funcName = compMethods[i].Name;
            string fullFuncName = funcName + "(" + compMethods[i].getParametersString() + ")";            
            if(splitString[1].Contains(fullFuncName)) 
            {
                methodInfo = compMethods[i];
                break;
            }
            else{
                methodInfo = null;
    ...
    public static void putSelectedValue(ref UnityEvent unityEvent, ref System.Object obj, ref List<Type> types, UnityEngine.Object unityObj, string selectedField)
    {
        ...
        try
        {
            ...
            if(fieldInfo != null) {
                ...
                if( !typeof(UnityEngine.Events.UnityEventBase).IsAssignableFrom( fieldType ) ) { return;}
                // unityEventBase = (UnityEventBase) fieldInfo.GetValue(unityObj);
                // Debug.Log( (UnityEvent) unityEventBase );
                // Debug.Log( fieldType.GetMethod("AddListener").ToString() );
    ...
    // public static void putSelectedValue(ref Component component, UnityEngine.Object unityObj, string selectedField)
    // {
    //     component = null;
    //     if(selectedField == "") return;
    //     string[] splitString = selectedField.Split('/');
    //     if(splitString[0] == "GameObject") 
    //     {
    //         component = null;
    //     }else
    //     {
    //         GetComponent(ref gm, ref component, splitString[0]);
    //     }
    // }
    #endregion
                                        ...
                                        string selectedField = "")
    {
        ...
        {
            // Debug.Log("all three are null " + selectedField);
            string[] splitString = selectedField.Split('/');
    ...
    public static System.Object[] GetValueFromParameters(Parameters[] parameters, Type[] parametersType)
    {
        ...
        for (int i = 0; i < parameters.Length; i++)
        {
            ...
            {
                // if GameObject Field  selected
                // bool isBothValueSame = ( parameters[i].selectField.Split('/')[0] == parameters[i].selectField.Split('/')[1]) ? true : false;

                // System.Object getValueFrom = null;
                // if(parameters[i].selectedComponent != null) { 
                //     if(isBothValueSame) 
                //         parameterList.Add( parameters[i].selectedComponent ); 
                //     else
                //         getValueFrom = parameters[i].selectedComponent; 
                // }
                // else if(parameters[i].unityObj != null) {
                //     if(isBothValueSame) 
                //         parameterList.Add( parameters[i].unityObj );
                //     else
                //         getValueFrom = parameters[i].unityObj; 
                // }
            ...
            // {
            //     Debug.Log("Param[" + i + "] => null");
            // }
            // else
            // {
            //     Debug.Log("Param[" + i + "] => " + parameterList[i]);
            // }   
            // #endif 
    ...
    public static void SetParametersValue(ref Parameters[] parameters, ParameterInfo[] methodParameters)
    {
        for (int x = 0; x < parameters.Length; x++)
        {
            ...
            // ReflectionExtension.putSelectedValue(ref parameters[x].selectFunctionParameters[x],parameters[x].selectedField,parameters[x].selectedProperty);
            

            // ReflectionExtension.putSelectedValue(ref parameters[x].selectedComponent, parameters[x].unityObj,parameters[x].selectField);
